...
Phaser.Plugin.PathFinderPlugin.prototype.init = function(baseLayer) {
	this.baseLayer = baseLayer;
	this.setGrid(baseLayer.layer.data, [-1]);
}

Phaser.Plugin.PathFinderPlugin.prototype.getTileXY = function(obj) {
	var tileX = this.baseLayer.getTileX(obj.x);
  var tileY = this.baseLayer.getTileY(obj.y);
  // debugger;
  // tileX  = tileX < 0 ? 0 :tileX;  //Hack to fix getting 'out of grid' errors when scroll leaves cat offscreen
  // tileY  = tileY < 0 ? 0 :tileY;  //Need to check tile coords are fetched relative to the world, not screen
  return [tileX, tileY];
}

Phaser.Plugin.PathFinderPlugin.prototype.getPixelXY = function (tileCoord) {
  var x, y;
  x = (tileCoord.x * 32) + 16;
  y = (tileCoord.y * 32) + 16;
...
Phaser.Plugin.PathFinderPlugin.prototype.getBoundsFromPoint = function(point){
  var x = Math.floor(point.x / 32) * 32;
  var y = Math.floor(point.y / 32) * 32;

  return new Phaser.Rectangle(x, y, 32, 32);
}
/**
